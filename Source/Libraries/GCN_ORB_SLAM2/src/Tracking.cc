...
cv::Mat Tracking::GrabImageStereo(const cv::Mat &imRectLeft, const cv::Mat &imRectRight, const double &timestamp)
{
    ...
    mCurrentFrame = Frame(mImGray,imGrayRight,timestamp,mpFeatureExtractorLeft,mpFeatureExtractorRight,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth);
...
cv::Mat Tracking::GrabImageRGBD(const cv::Mat &imRGB, const cv::Mat &imD, const double &timestamp)
{
    ...
    mCurrentFrame = Frame(mImGray,imDepth,timestamp,mpFeatureExtractorLeft,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth);
...
cv::Mat Tracking::GrabImageMonocular(const cv::Mat &im, const double &timestamp)
{
    ...
    if(mState==NOT_INITIALIZED || mState==NO_IMAGES_YET)
        mCurrentFrame = Frame(mImGray,timestamp,mpIniFeatureExtractor,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth);
    else
        mCurrentFrame = Frame(mImGray,timestamp,mpFeatureExtractorLeft,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth);
